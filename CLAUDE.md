...
| 2026-05-12 | **v4 — Production grade (Sprints 1-3)**. Hilo: hacer FAMA correr 30+ días sin intervención. Scope completo en `docs/v4/SCOPE-v4.md`. (1) **Sprint 1 — Observabilidad con Langfuse self-hosted**. Decisión: self-hosted en VPS (no Cloud). Setup operacional documentado en `docs/v4/SETUP-langfuse-selfhosted.md`. Cambios de código: `src/mastra/observability.ts` nuevo (construye `Observability` instance con `DefaultExporter` + `LangfuseExporter` cuando las keys están seteadas), `src/config/env.ts` con `LANGFUSE_BASE_URL` / `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` (todas opcionales). RequestContext en `src/server/webhook.ts` ahora setea `sessionId=chatwoot-{conversationId}` y `userId=contact-{contactId}` para que Langfuse agrupe traces. `requestContextKeys` config en observability extrae automáticamente `conversationId/contactId/phone/contactName/sessionId/userId` como metadata. En `NODE_ENV=test` se skipea toda la observability para no contaminar asserts. Dependencias nuevas: `@mastra/observability@^1.11.1`, `@mastra/langfuse@^1.2.5`. (2) **Sprint 2 — Resiliencia**. `src/lib/circuit-breaker.ts` nuevo — clase genérica con states closed/open/half-open + rolling failure window. `src/lib/background-tracker.ts` nuevo — contador global de promises in-flight para drain en shutdown. **LLM circuit** (`webhook.ts`): 3 fallos del `recepcionista.generate()` en 60s abre el circuit por 5 min. Mientras abierto: `llmCircuitOpenFallback()` postea mensaje fijo al cliente + escalation manual de 4 pasos (label/nota privada/asignación/toggle status) — NO usa la tool chatwoot-handoff porque requiere LLM. **Chatwoot circuit** (`src/lib/chatwoot.ts`): 3 fallos 5xx/network en 60s abre por 2 min. 4xx NO cuenta (son nuestros bugs, retry no arregla). `chatwootPost` lanza `ChatwootCircuitOpenError` cuando está abierto. **Graceful shutdown** (`src/mastra/index.ts`): SIGTERM/SIGINT → stop workers (nurturing, auto-handback, dedup) → drain background (10s timeout vía `waitForBackgroundDrain`) → flush observability (`mastra.observability.shutdown()`) → `process.exit(0)`. `docker-compose.yml` agrega `stop_grace_period: 30s` para dar margen sobre el 10s default de Docker. El webhook ahora envuelve el background processing con `trackBackground()`. 11 tests circuit-breaker + 5 tests background-tracker + 1 test integration (LLM fallback path). (3) **Sprint 3 — Eval set sistemático**. `tests/eval/runner.ts` — script standalone que mockea fetch para Chatwoot/Twenty/Calendar (200 vacío) pero deja pasar OpenAI (LLM real). 5 casos canónicos YAML iniciales en `tests/eval/cases/`: arquetipo-1-caliente, arquetipo-2-explorar, excepcion-1-pedido-humano, excepcion-3-urgencia, edge-anti-hallucination (regression del bug v3 #1). Aserciones soportadas: `toolsCalled`, `delegatedTo`, `toolArgsMatch`, `finalResponseMatches`, `finalResponseDoesNotMatch`. `hard: true` bloquea CI, `hard: false` solo advierte. GitHub Action `.github/workflows/eval.yml` corre en cada PR a `main`. Dependencias nuevas: `tsx@^4.21.0` (devDep) para ejecutar el runner. Costo estimado por full run: ~USD 0.10-0.50. Total suite: **223/223 verde**, typecheck limpio. **Pendiente Mariano**: (a) levantar Langfuse en el VPS siguiendo el setup doc, (b) pegar `LANGFUSE_*` env vars en Dockploy, (c) configurar `OPENAI_API_KEY` como GitHub secret para que el workflow de eval pueda correr, (d) en Settings → Branches de GitHub, marcar `eval` como required status check para `main`. Sprint 4 (operational controls — label no-bot, Zod, debouncing) queda diferido a v5. |
| 2026-05-12 | **v3 — 4 bugs de producción**. (1) **Bug #1 CRÍTICO — recepcionista alucina confirmaciones**: el recepcionista respondía "Quedó agendada la demo" con `steps=1` (sin delegar) al ver en Memory que el agendador había ofrecido slots y el cliente elegía uno. Fix: bloque "⛔ VERIFICACIÓN OBLIGATORIA" agregado al INICIO de las instructions de recepcionista.ts, antes de "# Identidad de FOMO". Explica la "trampa del historial de memoria" y prohíbe explícitamente completar narrativas de turnos anteriores. (2) **Bug #2 — agendador hardening**: instrucción AUTOCHECK agregada en "Reglas duras" del agendador para re-llamar `list-calendar-slots` si hay incertidumbre sobre qué `slotStartMs` usar. (3) **Bug #3 — webhook async**: `handleChatwootWebhook` ahora responde 202 inmediatamente después del dedup, y delega el procesamiento pesado (LLM + Chatwoot) a `processMessageBackground`. En test mode (`NODE_ENV=test`) sigue sincrónico para que los tests existentes pasen. Razón: Chatwoot interpreta timeout del webhook como error y flipea la conversación a `open`, bloqueando al bot. (4) **Bug #4 — auto-handback worker**: nuevo `src/lib/auto-handback-worker.ts`. Cada 15 min, lista conversaciones `open` en el inbox de FAMA, flipea a `pending` las que tienen más de 30 min de inactividad (threshold configurable). Nuevo endpoint en `src/lib/chatwoot.ts`: `listConversationsByStatus`. Registrado en `src/mastra/index.ts`. 6 tests nuevos en `tests/lib/auto-handback-worker.test.ts`. Logging de tool calls agregado al log de `recepcionista responded` (Issue 3). Ack post-handoff mejorado en backoffice instructions con gestión de expectativa de timing. Total suite: 207/207 verde. |
