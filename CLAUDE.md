...
**Auth**: service account JSON sin domain-wide delegation. Cada calendar (Mariano + Guille) comparte su Google Calendar con el email del SA con permiso "Make changes to events". Más simple que OAuth user-flow + sin necesidad de admin del Workspace.
...
4. Compartir cada calendar con el email del SA (Calendar.google.com → tu calendar → Share with specific people → "Make changes to events").
5. Pegar JSON completo en `.env` como `GOOGLE_CALENDAR_CREDENTIALS_JSON` (sola línea).
6. Pegar emails en `.env`:
   - `CALENDAR_IDS_TO_CHECK="marianoberton@fomo.com.ar,guille@fomo.com.ar"` (intersección de busy times)
   - `CALENDAR_PRIMARY=marianoberton@fomo.com.ar` (donde se CREAN los eventos; el otro va como `attendee`)
...
- [ ] Crear proyecto en `console.cloud.google.com` (si no tenés uno).
- [ ] APIs & Services → Library → habilitar **Google Calendar API**.
- [ ] IAM & Admin → Service Accounts → **Create service account** (nombre tipo `fama-agendador`). Permisos en el proyecto: ninguno especial — el SA accede a los calendars vía sharing explícito, no via roles del proyecto.
- [ ] En el SA recién creado → Keys → Add key → Create new key → JSON. **Bajar el archivo**.
- [ ] Pegar el JSON entero (en una sola línea) en `.env` como `GOOGLE_CALENDAR_CREDENTIALS_JSON=...`.
- [ ] Ir a `calendar.google.com` con la cuenta de Mariano:
  - Settings → Settings for my calendars → tu calendar (`marianoberton@fomo.com.ar`) → "Share with specific people or groups" → agregar el email del SA (algo como `fama-agendador@<proyecto>.iam.gserviceaccount.com`) con permiso **"Make changes to events"**.
- [ ] Repetir el sharing en el calendar de Guille con el mismo email del SA + mismo permiso.
- [ ] Pegar en `.env`:
  ```
  CALENDAR_IDS_TO_CHECK=marianoberton@fomo.com.ar,guille@fomo.com.ar
  CALENDAR_PRIMARY=marianoberton@fomo.com.ar
  ```
